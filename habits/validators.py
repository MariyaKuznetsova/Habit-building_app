...
def validate_associated_habit(associated_habit, reward):
    ...
    if associated_habit and reward:
        ...
        raise ValidationError(
            "Нужно выбрать либо указать что связанная привычка или добавить вознаграждение!"
        )
...
def validate_pleasant_habit(associated_habit):
    ...
    if associated_habit and (associated_habit.pleasant_habit is False):
        raise ValidationError(
...
def validate_reward(associated_habit, pleasant_habit, reward):
    ...
    if (pleasant_habit is True) and (associated_habit or reward):
        raise ValidationError(
            "Не может приятная привычка быть связанной привычкой или иметь вознаграждение!"
        )
